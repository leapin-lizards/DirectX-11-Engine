...
bool Graphics::InitializeDirectX(HWND hwnd)
{
	try
	{
		...
		spriteFont = std::make_unique<DirectX::SpriteFont>(this->device.Get(), L"comicsans_ms_16.spritefont");
...
bool Graphics::InitializeScene()
{
	try
	{
		HRESULT hr = CreateWICTextureFromFile(this->device.Get(), L"Data\\Textures\\grass.jpg", nullptr, this->grassTexture.GetAddressOf());
		COM_ERROR_IF_FAILED(hr, "Failed to create texture.");

		hr = CreateWICTextureFromFile(this->device.Get(), L"Data\\Textures\\pink.png", nullptr, this->pinkTexture.GetAddressOf());
		COM_ERROR_IF_FAILED(hr, "Failed to create texture.");

		hr = CreateWICTextureFromFile(this->device.Get(), L"Data\\Textures\\pavement.jpg", nullptr, this->pavementTexture.GetAddressOf());
		COM_ERROR_IF_FAILED(hr, "Failed to create texture.");
		...
		if (!gameObject.Initialize("Data\\Objects\\frog\\datboi.obj",this->device.Get(), this->deviceContext.Get(),  this->cb_vs_vertexshader))
		{
			return false;
		}
